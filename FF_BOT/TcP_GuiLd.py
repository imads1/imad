...
class FF_CLient():
    ...
    /ou/id ghost_name
    [ffffff]تابعني على التيك توك و الأنستا\n[FFD3EF]{bot_name}\n''', 1 , Guild_Uid , self.DeCode_CliEnt_Uid , key , iv))
                        time.sleep(0.5)
                        self.CLients.close() ; self.CLients2.close()
                        self.Connect_SerVer(Token , tok , host , port , key , iv , host2 , port2 , Guild_Uid , Auth_Code)	
                    ...
                    if f'{tag}' in self.name and client_id not in black and True == self.Zx or client_id in owner and client_id not in black and True == self.Zx or client_id in approve:      
        ...
        [ffffff]	
          ...
          LiKes GiVen : {xMsGFixinG(a5)}
           ...
           [90EE90]Dev : {bot_name}OfficieL\n''', 1 , Guild_Uid , self.DeCode_CliEnt_Uid , key , iv))
                    ...
                    id = self.InputMsg[6:]
                    self.Zx = ChEck_Commande(id)    	
                    if f'{tag}' in self.name and client_id not in black or client_id in owner and client_id not in black or client_id in approve and True == self.Zx: 	      
                ...
                if '/pp/' in self.InputMsg[:4]:
                        ...
                        self.id , self.nm = (self.InputMsg[4:].split(" ", 1) if " " in self.InputMsg[4:] else [self.InputMsg[4:], f"{bot_name}"])
                        self.Zx = ChEck_Commande(self.id)	    
                        if f'{tag}' in self.name and client_id not in black or client_id in owner and client_id not in black or client_id in approve and True == self.Zx:
                    ...
                    if f'{tag}' in self.name and client_id not in black or client_id in owner and client_id not in black or client_id in approve:
                        ...
                        if success_count > 0:
                            success_msg = f'\n[b][c][90EE90]Successfully sent emote  to  players!\n'
                ...
                elif '/jn' in self.InputMsg[:3]:
                    self.id = self.InputMsg[3:]	 
                    self.Zx = ChEck_Commande(self.id)
                    if f'{tag}' in self.name and client_id not in black or client_id in owner and client_id not in black or client_id in approve and True == self.Zx:
                ...
                elif '/vip/' in self.InputMsg[:5]:
                    self.id = self.InputMsg[5:]
                    self.Zx = ChEck_Commande(self.id)
                    if f'{tag}' in self.name and client_id not in black or client_id in owner and client_id not in black or client_id in approve and True == self.Zx:
                ...
                elif '/list' in self.InputMsg[:5] and self.InputMsg[:6] != '/list ':
                    if f'{tag}' in self.name and client_id not in black or client_id in owner and client_id not in black or client_id in approve:
                ...
                elif '/-vip/' in self.InputMsg[:6]:
                    del_id = self.InputMsg[6:] 
                    if f'{tag}' in self.name and client_id not in black or client_id in owner and client_id not in black or client_id in approve:
                    ...
                    tmcd = parts[0]
                    ghost_name = parts[1] if len(parts) > 1 else f"{bot_name}"
