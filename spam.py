...
class FF_CLient():
    ...
    def ToKen_GeneRaTe(self , Access_ToKen , Access_Uid):
        ...
        self.dT = bytes.fromhex('')
        
       
        self.dT = self.dT.replace(b'2026-04-08 19:23:23' , str(datetime.now())[:-7].encode())        
        self.dT = self.dT.replace(b'f7b53f72785049be6978604de2815c3cc7c017235f54eab41d8a349c18fb432c' , Access_ToKen.encode())
        self.dT = self.dT.replace(b'7f0d30b4c0c9663e83945f6f614d2985' , Access_Uid.encode())
...
def start_account(account):
    while True:  # استخدام حلقة تكرار بدلاً من الاستدعاء الذاتي
        try:
            print(f" Starting account: {account['id']}")
            FF_CLient(account['id'], account['password'])
            # إذا وصل الكود هنا وتوقف الاتصال بشكل طبيعي، سيخرج من الحلقة
            break 
        except Exception as e:
            # عند حدوث خطأ، سيطبع السبب وينتظر ثم يعيد المحاولة في نفس الحلقة
            print(f" Error starting account {account['id']}: {e}")
            time.sleep(20)  # زيادة وقت الانتظار لـ 20 ثانية لتجنب الحظر وانهيار المعالج  

def auto_restart_process():
    # الانتظار لمدة ساعة (3600 ثانية)
    time.sleep(3600) 
    print("\n[!] تنبيه: يتم الآن إعادة تشغيل السيرفر تلقائياً لتجديد الموارد...")
    # إغلاق العملية؛ سيقوم Hugging Face بإعادة تشغيل الـ Space فوراً
    os._exit(0)

def StarT_SerVer():
    # 1. تشغيل عداد إعادة التشغيل في الخلفية
    threading.Thread(target=auto_restart_process, daemon=True).start()

    # 2. تشغيل الـ API (Flask)
    ...
    # 4. الانتظار حتى تنتهي الخيوط (Threads)
    for thread in threads:
        thread.join()

# --- حلقة التكرار اللانهائية لضمان عدم توقف الملف نهائياً ---
if __name__ == "__main__":
    while True:
        try:
            print("🚀 بدء تشغيل السيرفر ونظام الحسابات...")
            StarT_SerVer()
        except Exception as e:
            print(f"❌ حدث خطأ غير متوقع: {e}")
        
        print("🔄 محاولة إعادة التشغيل بعد 10 ثوانٍ...")
        time.sleep(10)